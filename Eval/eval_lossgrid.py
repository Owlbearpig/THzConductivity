...
from functions import pearson_corr_coeff, zero_pad_fd
...
def main():
    ...
    n_vals, k_vals = [], []
    grid_vals = np.load(str(ROOT_DIR / "Plotting" / f"01 GaAs Wafer 25_2022_02_14_{sample_idx}_True_half_range.npy"))
    ...
    plot_ri(new_cost.n_approx, label=f"Analytical, avg. all measurements")
    ...
    mod_fd = tmm_from_ri(n, d_list, ref_fd, en_plot=False)
    mod_fd = mod_fd[freq_idx_slice, :]

    plot_field(mod_fd, label=f"TMM(n_grid_minima)")
    print(pearson_corr_coeff(mod_fd, sam_fd))
