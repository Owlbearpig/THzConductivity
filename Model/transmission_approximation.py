...
def ri_approx(ref_fd, sam_fd, thickness):
    ...
    n = (1 + phase_diff * c0 / (d * omega)).real

    t_func = sam_fd[:, 1] / ref_fd[:, 1]

    k = - c0 * np.log(np.abs(t_func) * (n + 1) ** 2 / (4 * n)) / (omega * d)

    return array([freqs, n.real + 1j * k.real]).T


def no_fp_mod(ref_fd, n, thickness):
    # simple slab, two interfaces, not considering fp
    d = thickness * um
    freqs = ref_fd[:, 0].real
    omega = 2 * pi * freqs * THz
    ...
    mod_fd = t12 * t21 * ref_fd[:, 1] * np.exp(1j * n[:, 1] * omega * d / c0)
    extra_ref_phase = np.exp(-1j * omega * d / c0)

    return array([freqs, mod_fd*extra_ref_phase]).T
...
def main():
    ...
    no_fp_mod_fd = no_fp_mod(avg_ref_fd, n, d_list[1])
